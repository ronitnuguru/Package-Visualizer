...
export default class ScratchRowMetaExpression extends LightningElement {
  ...
  getMetaRows() {
    const rows = this.fieldSettings.reduce((acc, row) => {
      ...
      return {
        ...acc,
        [row.fieldName]: castFieldValue(row.fieldOperator, row.fieldValue)
      };
    }, {});
  ...
  handleAddMetaSetting() {
    ...
    this.fieldSettings = [
      ...this.fieldSettings,
      decorate({ ...EMPTY_FIELD }, nextIndex)
    ];
  }
  ...
  handleMetadataValueChange(event) {
    ...
    this.fieldSettings = this.fieldSettings.map((row, i) =>
      i === index
        ? decorate({ ...row, fieldValue: event.target.value }, i)
        : row
    );
  }
  ...
  handleOperatorChange(event) {
    ...
    this.fieldSettings = this.fieldSettings.map((row, i) =>
      i === index
        ? decorate({ ...row, fieldOperator: operator, fieldValue: "" }, i)
        : row
    );
  }
}
