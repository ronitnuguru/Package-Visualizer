...
export default class ScratchDefFileBuildCard extends NavigationMixin(LightningElement) {
    ...
    addFeatures(features) {
        ...
        const existing = new Set(this.featuresList.map((f) => f.name.toLowerCase()));
        const additions = [];
    ...
    applyTemplate(event) {
        ...
        Object.entries(template.settings || {}).forEach(([settingName, fields]) => {
    ...
    handleResetForm() {
        ...
        const settingsCmp = this.template.querySelector('c-scratch-settings-expression');
        if (settingsCmp) {
            settingsCmp.reset();
        }
