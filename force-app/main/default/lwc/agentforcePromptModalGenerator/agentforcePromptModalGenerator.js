...
export default class AgentforcePromptModalGenerator extends LightningModal {
  ...
  get allModelsTypeOptions() {
    return [
      ...
      {
        provider: "OpenAI",
        label: "OpenAI GPT 4 Omni Mini (GPT-4o mini)",
        value: "sfdc_ai__DefaultOpenAIGPT4OmniMini"
      },
      {
        provider: "OpenAI",
  ...
  connectedCallback() {
    this.promptCurrentTime = new Date().toLocaleTimeString([], {
      hour: "numeric",
      minute: "2-digit",
      hour12: true
    });
    ...
    (async () => {
      await invokePromptAndUserModelsGenAi({
        ...
        .then((result) => {
          ...
          this.responseCurrentTime = new Date().toLocaleTimeString([], {
            hour: "numeric",
            minute: "2-digit",
            hour12: true
          });
